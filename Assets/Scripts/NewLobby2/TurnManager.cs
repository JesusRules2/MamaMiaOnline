...
public bool bMarioWins = true;
    ...
    [SyncVar] 
    public bool bGameOverRespawn = true;
    [SyncVar]
    bool bGameOverTimer;
    [SyncVar]
    bool bMarioCheck;
...
bool bMarioSpawn = false;
    ...
    public PlayerSwitch[] PlayersLeftSwitch;
...
float CreateMarioTimer = 0;
    ...
    IEnumerator ServerSpawn () {
        ...
        PlayersLeftSwitch = FindObjectsOfType<PlayerSwitch> ();

        foreach (PlayerSwitch player in PlayersLeftSwitch) {

            yield return new WaitForSeconds (0.001f);
            player.SwitchToPikachu ();

            // yield return new WaitForSeconds (0.010f);
            // player.SwitchToMario ();
    ...
    IEnumerator Update1() {
        ...
        if (bShowScreen) {
            LoadingScreen.SetActive(true); //turn off?
        }
        else {
        ...
        {   
            ...
            if (bPikachusSpawn == false && EndTimer > 7.4f)
            {
                PlayersLeftSwitch = FindObjectsOfType<PlayerSwitch> ();
        ...
        // SPAWN MARIO
        if (EndTimer > 9f && bMarioSpawn == false) //8
        {            
            CreateMarioTimer += Time.deltaTime;
            if (CreateMarioTimer > 2f) 
            {
                PlayersLeftSwitch[Random.Range(0,PlayersLeftSwitch.Length)].SwitchToMario();
                RespawnLevelUpdateAttempt();
                CreateMarioTimer = 0;
            }
